...
| `policy_comparison_metrics.csv` | Multi-metric comparison of no-defense, fixed-defense, and adaptive-hybrid policies. |
...
The policy-comparison CSV uses the same sampled-data timing as the environment: observe at `t_k`, apply any jump, integrate over `[t_k,t_{k+1})`, then measure the next observation.
