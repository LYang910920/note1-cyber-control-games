...
The companion plots are saved as `figures/training_iteration_diagnostics.png` and `figures/game_response_matrix.png`.
...
The policy-comparison and game-response CSV files use the same sampled-data timing as the environment: observe at `t_k`, apply any jump, integrate over `[t_k,t_{k+1})`, then measure the next observation.
