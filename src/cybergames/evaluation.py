...
PolicyFn = Callable[[SampledFlowImpulseEnv, int, np.ndarray], Action]
AttackerFn = Callable[[SampledFlowImpulseEnv, int, np.ndarray], Action]


def no_defense_policy(env: SampledFlowImpulseEnv, k: int, obs: np.ndarray) -> Action:
    return env.DEF_NONE


def always_patch_policy(env: SampledFlowImpulseEnv, k: int, obs: np.ndarray) -> Action:
    return mode_intensity(env.DEF_PATCH, 0.8)


def always_clean_policy(env: SampledFlowImpulseEnv, k: int, obs: np.ndarray) -> Action:
    return mode_intensity(env.DEF_CLEAN, 0.8)


def adaptive_sampled_impulse_policy(env: SampledFlowImpulseEnv, k: int, obs: np.ndarray) -> Action:
...
def scripted_attacker_policy(env: SampledFlowImpulseEnv, k: int, obs: np.ndarray) -> Action:
    """Scenario attacker used in the single-defender tutorial experiments."""
    return scripted_attacker(env, k)


def fixed_scan_attacker(env: SampledFlowImpulseEnv, k: int, obs: np.ndarray) -> Action:
    return env.ATK_SCAN


def fixed_exploit_attacker(env: SampledFlowImpulseEnv, k: int, obs: np.ndarray) -> Action:
    return env.ATK_EXPLOIT


def fixed_lateral_attacker(env: SampledFlowImpulseEnv, k: int, obs: np.ndarray) -> Action:
    return env.ATK_LATERAL


def fixed_stealth_attacker(env: SampledFlowImpulseEnv, k: int, obs: np.ndarray) -> Action:
...
def rollout_policy(
    ...
    env = (
        SampledFlowImpulseEnv(config=config, seed=seed)
        if config
        else SampledFlowImpulseEnv(seed=seed)
    )
...
def summarize_rollout(rollout: Dict[str, object]) -> Dict[str, float | int | str]:
    ...
    deception_steps = int(np.count_nonzero(defender_modes == SampledFlowImpulseEnv.DEF_DECEIVE))
    action_switches = (
