...
\begin{document}
\lecturetitle{Code Running Guide}{How to Run and Extend the Note 1 Python Examples}{Installation, smoke tests, training diagnostics, outputs, and troubleshooting}{Companion to the cyber control and game-learning lecture note}
\tableofcontents
\newpage
...
\begin{lstlisting}[style=pythonstyle]
python src/madrl_ctde_hybrid_game.py --episodes 200
\end{lstlisting}
...
\end{lstlisting}
Expected output: refreshed PNG figures under \texttt{figures/}, including neural architecture, FBSM, hybrid rollout, and policy-comparison plots.
...
\item the training-diagnostics and game-response PNG figures.
\end{itemize}
...
For larger games, keep the environment contract fixed: observe at $t_k$, apply jumps only when intended, integrate over the decision interval, and report cyber metrics alongside rewards. Then add opponent pools, multiple random seeds, and unilateral-deviation checks.
